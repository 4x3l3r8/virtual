...
if(isset($_POST['send'])){
	...
	$sid=$_POST['sid'];
	$msg=$_POST['text'];
	mysqli_query($conn, "insert into outbox (sender_id,sender_name,reciever_id,reciever_name,message,date,sender_category,reciever_category,status) values('$ses_id','$ses_name','$sid','$to','$msg','$date','lecturer','student','')");
	mysqli_query($conn, "insert into inbox (sender_id,sender_name,reciever_id,reciever_name,message,date,sender_category,reciever_category,status) values('$ses_id','$ses_name','$sid','$to','$msg','$date','lecturer','student','')");
...
if(($_GET['outbox_read'])){
	$rid=$_GET['outbox_read'];
			...
			<form enctype="multipart/form-data" action="" name="form" method="post">
				...
				<label>To: </label><input type="text" name="c_code" value="'.ucwords($col['reciever_name']).'" class="form-control" style="width:350px; height:39px;" readonly></br>
<textarea name="text" class="form-control" style="resize:none; width:400px; height:150px;" readonly>'.ucfirst($col['message']).'</textarea><br></div><br></br>';
}
?>
...
<span class="normalFont"><a href="logout.php" class="btn btn-success navbar-right navbar-btn"><strong>Sign Out</strong></a></span>
		...
		<label>To: </label><input type="text" class="form-control" name="to" value="<?php echo ucwords($row['username']); ?>"style="width:400px; height:38px;" readonly><br>
		...
		<input type="submit"  class="btn btn-primary" name="send" value="Send" style="width:20%;"></div>
			...
			<?php
			$no=0;
			$query=mysqli_query($conn, "select * from outbox  where sender_id='$ses_id' AND sender_category='lecturer' order by date desc");
			...
			<?php
			$no=0;
			$query=mysqli_query($conn, "select * from inbox where reciever_id='$ses_id' AND sender_category='student' AND reciever_category='lecturer' order by date desc");
			if(mysqli_num_rows($query)>0){
			while($row=mysqli_fetch_assoc($query)){
				$no=$no+1;
			if($row['status']==''){
