...
if(isset($_GET['del'])){
	$qid=$_GET['del'];
	mysqli_query($conn, "delete from student_score where course='$qid'");
	$msg='</br><p style="color:crimson;">Results deleted successfully!</p>';
}
...
<a href="home.php" >Groups</a>		
  ...
  <a href="quiz_result.php" class="active">Quiz Results</a>		
...
<span class="normalFont"><a href="logout.php" class="btn btn-success navbar-right navbar-btn"><strong>Sign Out</strong></a></span>
		...
		<div style="margin:-1%;"><h3 style="text-align:center;">Quiz results</h3></div><hr></br>
		<div align="center">
				<form method="get">
				<select name="text" class="form-control" style="width:50%; height:39px;" required>
				<option value="">--Select Group Quiz--</option>
				<?php
				$qry=mysqli_query($conn, "select * from course where lecturer_id='$lid'");
				while($row=mysqli_fetch_assoc($qry)){
				echo '<option value="'.$row['course_code'].'">'.strtoupper($row['course_code']).'</option>';
				...
				if(isset($_GET['search'])){
					...
					echo '<div style="margin-bottom:-12%; margin-top:10%;"><a href="quiz_result.php?del='.$_GET['text'].'" class="btn btn-info">Empty '.strtoupper($_GET['text']).' Results</a></div>';
					ECHO '<div align="center" style="margin-bottom:2%; margin-top:12%;"><H4>'.strtoupper($_GET['text']).' Group Quiz Results</H4></div>';
					echo '<table cellpadding="0" cellspacing="0" border="0" class = "table table-bordered" id="example">
			<thead>
				<tr><th align="center">S/N</th>
					<th width="40%" align="center">Username</th>
					<th width="40%" align="center">Matric No</th>
					<th width="40%" align="center">Course</th>
					...
					$text=$_GET['text'];
					$no=0;
					$query=mysqli_query($conn, "select * from student_score where course='$text'");
					if(mysqli_num_rows($query)>0){
						while($col=mysqli_fetch_array($query)){
							$no=$no+1;
						echo '<tr><td>'.$no.'</td><td>'.ucwords($col['username']).'</td>
						<td>'.$col['matric_no'].'</td>
						<td>'.$col['course'].'</td>
						<td>'.$col['score'].'</td>
						<td>'.$col['date'].'</td>
