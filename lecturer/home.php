...
<span style="font-size:30px; cursor:pointer; color:white; float:left; margin-left:5%; padding-top:3px;" class="menu" title="Menu" onclick="openNav()">&#9776;</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <a href="home.php" class="active">Groups</a>		
...
<span class="normalFont"><a href="logout.php" class="btn btn-success navbar-right navbar-btn"><strong>Sign Out</strong></a></span>
		...
		<h2 style="font-size:22px; text-align:center;">You are the Coordinator of the following group(s)</h2><hr>
				<?php
				$qry=mysqli_query($conn, "select * from course where lecturer_id='$ses_id'");
				if(mysqli_num_rows($qry)>0){
					while($row=mysqli_fetch_array($qry)){
						$no=0;
						$c_code=$row['course_code'];
						$sql=mysqli_query($conn, "select * from registered_group where group_name='$c_code'");
						$count=mysqli_num_rows($sql);
				echo '<button class="accordion" style="border-bottom:3px solid gray;"><img src="../img/1-512.png" width="5%">&nbsp;&nbsp;'.strtoupper($row['course_code']).' Group
						<font style="float:right;">'.$count.' member(s)</font>
						</button> <div class="panel">
						<h3 style="text-align:center; font-size:18px;">Group Members</h3>
						<table border="1"><tr><th>S/N</th><th>Username</th><th>Reg Date</th><th>Profile</th><th>Message</th><th>Delete</th></tr>';
						while($sol=mysqli_fetch_array($sql)){
							$no=$no+1;
							echo '
							<tr><td>'.$no.'</td><td>'.ucwords($sol['student_username']).'</td><td>'.$sol['reg_date'].'</td>
							<td><a href="student_profile.php?profile='.$sol['student_id'].'"><img src="../img/account_card-512.png" width="9%"></a></td>
							<td><a href="message.php?message='.$sol['student_id'].'"><img src="../img/email send.ico" width="17%"></a></td>
							<td><a href="home.php?del='.$sol['student_id'].'"><img src="../img/delete.gif"></a></td>
							</tr>';
