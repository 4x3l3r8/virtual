...
if(isset($_POST['post'])){
	...
	$group=$_POST['course'];
	mysqli_query($conn, "insert into forum(forum,member_id,member_name,member_category,post,date) values('$group','$sid','$sname','','$message','$date')");
}
...
<span class="normalFont"><a href="logout.php" class="btn btn-success navbar-right navbar-btn"><strong>Sign Out</strong></a></span>
		...
		if(mysqli_num_rows($result)>0){
			...
			while($row=mysqli_fetch_array($result)){
				...
				//to check if its my post
				if($row['member_name']==$sname && $row['member_id']==$sid){
					$use='me';
				}
				else{
					$use=$row['member_name'];
				...
				//to unable user to delete their post
				if($row['member_id']==$sid && $row['member_category']==''){
...
<div class="bus" align="center" style="padding:5px;">
			...
			<font style="color:white; font-size:15px; font-family:Arial;"><?php echo ucfirst($row['post']); ?></font></div>
			<div style="color:skyblue; font-weight:600; margin-bottom:-1%; margin-top:1%;"><font style="margin-left:-1%;"><?php echo ucwords($use)?></font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
