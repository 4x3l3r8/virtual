...
if(isset($_GET['exit'])){
	$exit=$_GET['exit'];
	mysqli_query($conn, "delete from registered_group where student_id='$ses_id' AND group_name='$exit'");
		echo '<script>
		alert("You have successfully remove yourself from '.$exit.' Group");
		location.href="joined.php"
		</script>';
...
<span class="normalFont"><a href="logout.php" class="btn btn-success navbar-right navbar-btn"><strong>Sign Out</strong></a></span>
			...
			<div class = "col-lg-6 well" style="margin-bottom:5%; border:1px solid black; box-shadow: 0 4px 8px 0 black; opacity:0.95;">
				<h2 style="font-size:22px; text-align:center;">You are a member of the following group</h2><hr>
				<?php
		...
		<a href="chatexe.php?chat='.$row['group_name'].'" style="float:left; font-size:15px;" class="btn btn-success">&#128229; Login</a>
		<img src="img/Icons-Land-Vista-People-Groups-Meeting-Light.ico" width="5%">&nbsp;&nbsp;'.strtoupper($row['group_name'].' Group').'<a href="joined.php?exit='.$row['group_name'].'" style="float:right; font-size:16px;" class="btn btn-danger">Exit Group</a></h4>
						</div>';
					}
				}else{
					echo 'No Group Found !';
